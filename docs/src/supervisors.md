...
In the depicted case the supervisor `A10` has six child actors `A1`-`A6`. We setup this system:
...
Now, what the supervisor does if one of them – say `A4` - exits, is determined by its supervision strategy and by the child's restart variable and exit reason. With the default supervision strategy `:one_for_one` only the failed actor gets restarted:
...
With `:one_for_all` all supervised actors/tasks get restarted if one of them fails. That allows to restart a group of equitable actors depending on each other.
...
Supervisors allow for more automation and control of error handling in an actor system. They have the following API:
